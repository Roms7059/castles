...
class Furnace(Building):
    def __init__(self, x, y, image=None, upgrades=None):
        ...
        self.production_rate = self.base_production_rate * (1 + self.upgrades.get("production_rate", 0) * prod_multiplier)
        self.storage_capacity = self.base_storage_capacity + self.upgrades.get("storage_capacity", 0) * cap_increment
